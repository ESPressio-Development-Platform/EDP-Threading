...
6. Once `IsExecutionQuiescent()` is true, `FinalizeShutdown()` tears down every infrastructure context and publishes terminal completion. The runtime owns its bounded shutdown-completion wait surface; applications use `WaitForShutdown()`, `WaitForShutdownFor()` or `WaitForShutdownUntil()`.
